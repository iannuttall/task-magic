...
graph TD
    ...
    B1 -- No --> C;
    B2 --> C["Update TASKS.md with ALL Planned Tasks (Parent Tasks & Sub-tasks)"];
    C --> D["For Each Task (Parent Task or Sub-task) in TASKS.md"];
    D -- Loop --> E[Create/Update Individual task file in tasks/];
    ...
    S{User asks to archive?} --> T[Agent finds completed/failed tasks in tasks/];
    T --> U[Move task files to memory/tasks/];
    U --> V[Append summary to memory/TASKS_LOG.md];
    V --> W[Remove corresponding entries from TASKS.md];
```
